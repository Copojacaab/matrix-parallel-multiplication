 ...
 * 1) l'utente imposta m, n, p e genera griglie
 * 2) compila le celle Avvia calcolo (preferiamo le griglie se presenti e valide).
 * 3) validazione 
 * 4) POST al backend con matrixA, matrixB
 */
...
let activeMode = null; //'grids' || 'textarea'
...
/**
 * converte testoin array<rray<number>>
 * converte testo in array<array<number>>
 * accetta spazi, tab, virgole. 
 */
...
/** scrive una matrice M nella griglia */
...
/** prova a leggere dimensioni griglie dal DOM . */
...
/** (preferisce griglie, sennò textarea). */
function compatSummary() {
  // prova griglie
  const { aRows, aCols, bRows, bCols } = currentGridDims();
  if (activeMode === 'grids' && aRows && aCols && bRows && bCols) {
...
// riempimenti rapidi
[gridAFillZeros, gridAFillOnes, gridBFillZeros, gridBFillOnes].forEach(btn => {
  btn?.addEventListener('click', () => setMode('grids'));
});
...
// ------------------------
// aggiornamento live textarea
// ------------------------
function liveUpdate() {
  // se user digta in txtare attivo quella
  setMode('textarea');
  try {
    const A = parseMatrix(tA.value);
...
btnStart.addEventListener('click', async () => {
  ...
  try {
    if(activeMode === 'grids') {
      if(!(aRows && aCols && bRows && bCols))
        throw new Error('Griglie non presenti/complete');
      A = readGrid(gridA, aRows, aCols);
      B = readGrid(gridB, bRows, bCols);
    } else { //default: txtarea
  ...
  // 3) payload con matrici (cambio in db)
