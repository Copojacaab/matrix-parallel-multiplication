...
int main(int argc, char *argv[]) 
{
    ...
    if (taskid > MASTER)
    {
        ...
        // tutti i worker ricevono b
        b = alloc_matrix(nca, ncb);

        if(rows == 0){
            // buffer nullo per consumare il messaggio di A
            MPI_Recv(NULL, 0, MPI_DOUBLE, MASTER, mtype, MPI_COMM_WORLD, &status);
            // ricevo b normale
            MPI_Recv(b[0], nca * ncb, MPI_DOUBLE, MASTER, mtype, MPI_COMM_WORLD, &status);

            // rimando offseterows e nessun dato
            mtype = FROM_WORKER;
            MPI_Send(&offset, 1, MPI_INT, MASTER, mtype, MPI_COMM_WORLD);
            MPI_Send(&rows, 1, MPI_INT, MASTER, mtype, MPI_COMM_WORLD);

            MPI_Send(NULL,  0, MPI_DOUBLE, MASTER, mtype, MPI_COMM_WORLD);
